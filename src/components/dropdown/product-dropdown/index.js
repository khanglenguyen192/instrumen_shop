          ...
          <Consumer>
            {value => {
              return this.state.category.map(item => {
                return (
                  <DropdownMenuItem>
                    <ItemLink to="/product_list" onClick={() => { value.setProducts(item.categoryID); value.setTitle(item.categoryName); value.setTitleImg(item.titleImg); value.unSetFilterByPrice(); value.unSetIsProduct(); value.setBrand() }}>
